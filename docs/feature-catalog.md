...
| 功能 | 状态 | 主要入口 | 说明 |
| --- | --- | --- | --- |
| 通用占卜 Agent V1 | partial | `/api/app/agent/*` | Phase A/B/C/D/E/F 已实现 Agent 会话、占术路由预览、入口上下文、确认按钮、工具注册表、工具执行协议、Agent SSE、反馈、记忆控制、本地工具 provider 和工具驱动回答编排；专业算法/外部 provider 后续替换 |
