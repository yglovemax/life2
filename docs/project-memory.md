...
新的产品主线是通用占卜 Agent：在现有后端能力之上增加统一 Agent 编排层，让用户直接提问，由系统根据入口、用户选择和问题类型调用合适的八字、占星、塔罗、六爻、合盘、签文等能力。主 PRD 见 `docs/requirements/Nexa通用占卜Agent_PRD_V1.md`。
...
- 通用占卜 Agent V1 不是重做旧系统，而是在现有用户、聊天、记忆、模块渲染、知识库、算法库和模型路由上新增 Agent 路由与工具编排层。

## 下一步建议

1. 按 `Nexa通用占卜Agent_PRD_V1.md` 开发 Agent 路由和会话闭环。
2. 新增 Agent 工具注册表，把八字、占星、塔罗、六爻、合盘、签文接成可调用工具。
3. 优化记忆相关性筛选，避免无关记忆反复进入回答。
4. 增加 Agent 反馈和推荐闭环。
5. 在服务器上部署 Redis、Postgres + pgvector，跑 API/worker/向量检索生产冒烟。
6. 接真实八字排盘服务，补大运、流年、十神、旺衰等结构。
